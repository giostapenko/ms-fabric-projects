...
df = spark.read.option("multiline", "true").json("Files/data/ine_data/json/fruit_consumption/*.json")
display(df)
...
for year in years:
    ...
    if year in df.schema["Dados"].dataType.fieldNames():
        ...
        exploded_dfs.append(
            exploded_df.select(
                col("year"),
                col("record.dim_3_t").alias("species"),
...
final_df.createOrReplaceTempView('tab_fruit_consumption')
...
# MAGIC %%sql
# MAGIC CREATE TABLE tb_fruit_consumption AS
# MAGIC SELECT  DISTINCT
# MAGIC         SUBSTRING(year,1,4) as year,
# MAGIC         species,
# MAGIC         consumption
# MAGIC 
# MAGIC FROM tab_fruit_consumption
# MAGIC 
# MAGIC ORDER BY year, species 
