...
df = spark.read.option("multiline", "true").json("Files/data/ine_data/json/workload/*.json")
display(df)
    ...
    if year in df.schema["Dados"].dataType.fieldNames():
        ...
        exploded_dfs.append(
            exploded_df.select(
                ...
                col("record.dim_3_t").alias("labour_force_type"),
...
final_df.createOrReplaceTempView("tab_ine_workload")
...
# MAGIC %%sql
# MAGIC CREATE TABLE tb_workload AS
# MAGIC SELECT *
# MAGIC 
# MAGIC FROM tab_ine_workload
