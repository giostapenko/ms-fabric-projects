...
df = spark.read.option("multiline", "true").json("Files/data/ine_data/json/egg_prod/*.json")
display(df)
...
years = [f"{year}" for year in range (1983, 2023)]
...
final_df.createOrReplaceTempView("tb_ine_egg_production")
...
# MAGIC %%sql
# MAGIC CREATE TABLE tb_egg_production AS
# MAGIC SELECT *
# MAGIC 
# MAGIC FROM tb_ine_egg_production
