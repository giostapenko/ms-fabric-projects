...
df = spark.read.option("multiline", "true").json("Files/data/ine_data/json/egg_consumption/*.json")
display(df)
...
years = [str(year) for year in range(1983, 2022)]
...
final_df.createOrReplaceTempView('tab_ine_egg_consumption')
...
# MAGIC %%sql
# MAGIC CREATE TABLE tb_ine_egg_consumption AS
# MAGIC SELECT *
# MAGIC 
# MAGIC FROM tab_ine_egg_consumption
